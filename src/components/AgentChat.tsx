...
const WELCOME_FR = "Bonjour ! Je suis l'agent IA de Gestion Chantier Pro. Tu peux écrire, parler, ou joindre une photo de chantier pour demander conseil."
const WELCOME_EN = "Hello! I'm the AI agent for Gestion Chantier Pro. You can type, speak, or attach a job site photo to get expert advice."
...
function renderText(text: string) {
  return text.split('\n').map((line, i) => (
    <p key={i} className="mb-1 whitespace-pre-wrap break-words">{line || ' '}</p>
  ))
}
...
export default function AgentChat() {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { id: 'welcome', role: 'agent', content: WELCOME_FR },
  ])
  ...
  // Scroll to bottom & focus when opened
  useEffect(() => {
    if (open) {
  ...
  // Update welcome message when UI language changes (only if chat not started)
  useEffect(() => {
    setMessages(prev => {
      if (prev.length === 1 && prev[0].id === 'welcome') {
        return [{ id: 'welcome', role: 'agent', content: lang === 'fr' ? WELCOME_FR : WELCOME_EN }]
      }
      return prev
    })
  }, [lang])
  ...
  // ── Drag ──────────────────────────────────────────────────────────────────
  function savePos(next: Pos) {
    setPos(next)
    try { localStorage.setItem('ai-button-pos-v1', JSON.stringify(next)) } catch {}
  }
  ...
  function pointerMove(e: React.PointerEvent<HTMLButtonElement>) {
    ...
    if (!d || !ready) return
    const next = {
      x: clamp(e.clientX - d.dx, 8, window.innerWidth - 66),
      y: clamp(e.clientY - d.dy, 72, window.innerHeight - 82),
    }
    if (Math.abs(next.x - pos.x) > 2 || Math.abs(next.y - pos.y) > 2) d.moved = true
    savePos(next)
  }
  function pointerUp() {
  ...
  function clearConversation() {
    stopSpeech()
    setMessages([{ id: 'welcome', role: 'agent', content: lang === 'fr' ? WELCOME_FR : WELCOME_EN }])
    setSessionId(null)
    setImage(null)
  }
  ...
  async function sendMessage(textOverride?: string) {
    ...
    if (image && !aiSettings.allowPhotoAnalysis) {
      setMessages(prev => [...prev, { id: `warn-${Date.now()}`, role: 'agent', content: 'Analyse photo désactivée dans les réglages IA.' }])
      return
    }
    ...
    setMessages(prev => [
      ...prev,
      { id: userId,  role: 'user',  content: safeText, imageUrl: attachedImage?.previewUrl },
      ...
      const res = await fetch('/api/agent/chat', {
        ...
        body: JSON.stringify({
          ...
          aiSettings: {
            ...
            allowPhotoAnalysis: aiSettings.allowPhotoAnalysis,
          },
          userContext: {
            role: currentEmployee ? 'employee' : 'admin',
            name: currentEmployee?.name ?? (lang === 'en' ? 'Administrator' : 'Administrateur'),
  ...
  const btnStyle   = {
    left: ready ? pos.x : undefined, top: ready ? pos.y : undefined,
    right: ready ? undefined : '1rem', bottom: ready ? undefined : '6rem',
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed z-50 flex h-14 w-14 items-center justify-center rounded-full transition-all hover:scale-110 active:scale-95 focus:outline-none"
        style={btnStyle}
      >
      ...
      {/* Chat panel */}
      {open && (
        <div className="fixed z-50 flex flex-col rounded-3xl border border-amber-400/20 backdrop-blur-xl" style={panelStyle}>

          {/* Header */}
          <div
            className="flex flex-shrink-0 items-center gap-2 rounded-t-3xl border-b border-amber-400/15 px-4 py-3"
            style={{ background: 'linear-gradient(135deg, rgba(251,191,36,0.13), rgba(217,119,6,0.06))' }}
          >
            <div className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full text-lg" style={{ background: 'linear-gradient(135deg, #fbbf24, #d97706)' }}>
              🤖
            </div>
          ...
          <div className="flex-1 space-y-3 overflow-y-auto px-4 py-3">
            {messages.map(m => (
              <div key={m.id} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                ...
                >
                  {m.imageUrl && (
                    <img src={m.imageUrl} alt="Photo" className="mb-2 max-h-44 rounded-xl object-cover" />
                  )}
                  {m.content
          ...
          <div className="flex flex-shrink-0 gap-2 rounded-b-3xl border-t border-white/[0.07] p-3">
            <AgentImagePicker
              disabled={loading || !aiSettings.allowPhotoAnalysis}
              onImage={img => {
                setOpen(true)
                setImage(img)
                if (!input.trim()) setInput("Analyse cette photo de chantier et donne-moi les causes possibles, les risques, et les solutions.")
              }}
            />
            <AgentVoiceControls
              disabled={loading || !aiSettings.allowVoice}
              onText={text => setInput(text)}
              onVoiceEnd={text => { setInput(text); sendMessage(text) }}
            />
            <textarea
              ref={inputRef}
              rows={1}
