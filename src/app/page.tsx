  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      <style>{`
        ...
        .rf{animation:rFloat 3s ease-in-out infinite}
        .rw{animation:rWave 1.2s ease-in-out infinite;transform-origin:88px 60px}
        .re{animation:rBlink 3.5s ease-in-out infinite;transform-origin:center}
        .rg{animation:rGlow 2.5s ease-in-out infinite}
        .bp{animation:bPop 0.3s ease-out forwards}
      `}</style>
      <div style={{ position: 'relative', width: 110, height: 130 }}>
        {/* Bulle */}
        <div className="bp" style={{
          position: 'absolute', top: 0, left: '50%', transform: 'translateX(-50%)',
          background: happy ? 'linear-gradient(135deg,#7c3aed,#a855f7)' : ok ? '#4c1d95' : '#1e1b4b',
          border: `1px solid ${glowColor}`,
          borderRadius: '12px', padding: '5px 12px', fontSize: '11px', fontWeight: 800,
          color: '#e9d5ff', whiteSpace: 'nowrap', zIndex: 10,
          boxShadow: `0 0 14px ${glowColor}55`,
        }}>
          {message}
          <div style={{ position: 'absolute', bottom: -7, left: '50%', transform: 'translateX(-50%)', width: 0, height: 0, borderLeft: '7px solid transparent', borderRight: '7px solid transparent', borderTop: `7px solid ${happy ? '#a855f7' : '#4c1d95'}` }}/>
        </div>
        {/* Robot */}
        <svg className="rf rg" width="110" height="118" viewBox="0 0 110 118" fill="none" style={{ position: 'absolute', bottom: 0 }}>
          ...
          <g className="re" style={{ animationDelay: '0.18s' }}>
          ...
          <g className="rw">
            ...
            <line x1="105" y1="62" x2="109" y2="52" stroke="#a855f7" strokeWidth="2.5" strokeLinecap="round"/>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      ...
      <svg className="wbody" width="80" height="110" viewBox="0 0 80 110" fill="none">
        ...
        <g className="warm" style={{ animationDelay: '0.3s' }}>
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', paddingTop: '8px' }}>
        <style>{`
          ...
          .ec:active{transform:scale(0.97)}.ec{transition:transform 0.15s ease}
          .stw{animation:starT 2s ease-in-out infinite}
        `}</style>
  ...
  if (screen === 'pin') {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '20px', paddingTop: '16px' }}>
        ...
        {isXP ? (
          <>
            <div style={{ width: '76px', height: '76px', borderRadius: '18px', background: `linear-gradient(135deg, ${emp?.color}, #a855f7)`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '30px', fontWeight: 800, color: 'white', boxShadow: `0 0 30px ${emp?.color}66, 0 0 60px rgba(168,85,247,0.3)` }}>{emp?.name[0].toUpperCase()}</div>
            <div style={{ textAlign: 'center' }}>
              <p style={{ color: '#e9d5ff', fontSize: '17px', fontWeight: 800, letterSpacing: '2px' }}>{emp?.name}</p>
              <p style={{ color: '#6b21a8', fontSize: '11px', letterSpacing: '1px', marginTop: '2px' }}>ENTREZ VOTRE CODE</p>
            </div>
          </>
        ) : (
          <>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', paddingBottom: '8px' }}>
      <style>{`
        ...
        .sd{animation:sPulse 2s ease-in-out infinite}
        .np{animation:neonPulse 2.5s ease-in-out infinite}
        .nr{animation:neonRed 2s ease-in-out infinite}
        .goalInput:focus{animation:inputGlow 1.5s ease-in-out infinite!important}
      `}</style>
      ...
      {showCelebration && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 200, pointerEvents: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          {Array.from({ length: 24 }).map((_, i) => (
            <div key={i} style={{ position: 'absolute', left: `${Math.random() * 100}%`, top: '-20px', fontSize: '22px', animation: `confettiF ${1 + Math.random() * 2}s ease-in ${Math.random() * 0.5}s forwards` }}>
          ...
          <div>
            ...
            {isXP && goal ? (
              <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                <span style={{ fontSize: '9px', background: 'rgba(168,85,247,0.3)', color: '#a855f7', padding: '1px 6px', borderRadius: '4px', fontWeight: 800 }}>Nv.{goal.level}</span>
                <span style={{ fontSize: '9px', color: '#6b7280' }}>{goal.xpPoints} XP</span>
                {goal.streak > 0 && <span style={{ fontSize: '9px', color: '#f59e0b' }}>🔥{goal.streak}j</span>}
              </div>
            ) : (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '3fr 2fr', gap: '10px' }}>
        <div className={cardClass} style={{ ...card, background: isXP ? 'linear-gradient(135deg, #0a0514, #12082a)' : 'linear-gradient(135deg, #1a1200, #2a1f00, #1a1200)' }}>
          ...
          {revenue > 0 && [{ left: '15%', delay: '0s' }, { left: '50%', delay: '0.9s' }, { left: '80%', delay: '1.6s' }].map((coin, i) => (
            <span key={i} className="cf" style={{ left: coin.left, top: 0, animationDelay: coin.delay }}>{isXP ? '💜' : '🪙'}</span>
          ))}
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
              ...
              <div style={{ textAlign: 'right' as const }}>
                ...
                <p style={{ color: '#6b7280', fontSize: '10px' }}>{xpToNext} XP → Nv.{goal.level + 1}</p>
              </div>
            </div>
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            ...
            <div style={{ marginBottom: '16px' }}>
              ...
              <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
                ...
                <input
                  className="goalInput"
                  type="number"
                  value={editingGoal ? tempGoal : goal.targetAmount}
                  onChange={e => { setTempGoal(e.target.value); setEditingGoal(true) }}
                  onBlur={() => {
                    if (currentEmployeeId && tempGoal && parseFloat(tempGoal) > 0) {
                      setGoalTarget(currentEmployeeId, parseFloat(tempGoal))
                    }
                    setEditingGoal(false)
                  }}
                  style={{
                    width: '100%',
                    background: editingGoal ? 'rgba(34,211,238,0.12)' : 'rgba(34,211,238,0.06)',
                    border: editingGoal ? '2px solid #22d3ee' : '1px solid rgba(34,211,238,0.35)',
                    borderRadius: '14px',
                    ...
                    boxShadow: editingGoal ? '0 0 24px rgba(34,211,238,0.35)' : 'none',
                    transition: 'all 0.25s',
                  }}
                />
                <span style={{ position: 'absolute', right: '16px', fontSize: '13px', color: '#4c1d95', fontWeight: 700 }}>CAD</span>
              </div>
              <p style={{ fontSize: '11px', color: '#4c1d95', marginTop: '6px', textAlign: 'center' }}>
            ...
            <div style={{ marginBottom: '12px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '8px' }}>
                <div>
                  <p style={{ fontSize: '20px', fontWeight: 900, color: '#22c55e', lineHeight: 1 }}>{formatCurrency(goal.currentAmount)}</p>
                  <p style={{ fontSize: '10px', color: '#6b7280', marginTop: '2px' }}>gagné cette semaine</p>
                </div>
                <div style={{ textAlign: 'right' as const }}>
                  <p style={{ fontSize: '24px', fontWeight: 900, color: '#22d3ee', lineHeight: 1 }}>{goalPct.toFixed(0)}%</p>
                  <p style={{ fontSize: '10px', color: '#6b7280', marginTop: '2px' }}>complété</p>
                </div>
              </div>
              <XPBar current={goal.currentAmount} max={goal.targetAmount} color="#22d3ee"/>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '4px' }}>
                <span style={{ fontSize: '9px', color: '#4c1d95' }}>$0</span>
            ...
            <div style={{ padding: '10px 14px', borderRadius: '10px', background: goalPct >= 100 ? 'rgba(34,197,94,0.15)' : goalPct >= 50 ? 'rgba(34,211,238,0.1)' : 'rgba(168,85,247,0.08)', border: `1px solid ${goalPct >= 100 ? 'rgba(34,197,94,0.3)' : goalPct >= 50 ? 'rgba(34,211,238,0.25)' : 'rgba(168,85,247,0.15)'}` }}>
              <p style={{ fontSize: '12px', fontWeight: 700, color: goalPct >= 100 ? '#22c55e' : goalPct >= 50 ? '#22d3ee' : '#a855f7', margin: 0 }}>
                {goalPct >= 100 ? '🏆 OBJECTIF ATTEINT! Tu es une LÉGENDE!'
                  : goalPct >= 80 ? `🔥 Presque! Il te manque ${formatCurrency(goal.targetAmount - goal.currentAmount)}`
                  : goalPct >= 50 ? `💪 À mi-chemin! Encore ${formatCurrency(goal.targetAmount - goal.currentAmount)}`
                  : goalPct > 0 ? `⚡ En route! ${formatCurrency(goal.targetAmount - goal.currentAmount)} restants`
                  : '🎯 Poinçonne pour commencer ta progression!'}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className={cardClass} style={{ ...card }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '3px', marginBottom: '6px', position: 'relative', zIndex: 1 }}>
          {(lang === 'fr' ? ['DI','LU','MA','ME','JE','VE','SA'] : ['SU','MO','TU','WE','TH','FR','SA']).map(d => (
            <div key={d} style={{ textAlign: 'center' as const, fontSize: '9px', color: isXP ? '#4c1d95' : 'var(--primary)', fontWeight: 800, padding: '3px 0' }}>{d}</div>
          ))}
        </div>
